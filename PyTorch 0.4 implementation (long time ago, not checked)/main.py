...
def loss_fn(y_pred, y):
    ...
    loss = F.l1_loss(y_pred, y[0].repeat((1,n)).reshape((-1,1)))/scale  #
    return loss
...
def run(args):
    ...
    def log_testing_results(engine):
        if args.test_ratio > 0 and args.test_during_training:
    ...
    def final_testing_results(engine):
        if args.test_ratio > 0:
            ...
            np.save(args.save_result_file, (SROCC, KROCC, PLCC, RMSE, MAE, OR))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--decay_interval', type=int, default=1000,
                        help='learning rate decay interval (default: 1000)')
    parser.add_argument('--decay_ratio', type=int, default=0.1,
                        help='learning rate decay ratio (default: 0.1)')

    parser.add_argument('--exp_id', default=2000, type=int,
                        help='exp id for train-val-test splits (default: 0)')
    parser.add_argument('--train_ratio', type=float, default=0.6,
                        help='train ratio (default: 0.6)')
    parser.add_argument('--test_ratio', type=float, default=0.2,
                        help='test ratio (default: 0.2)') # val_ratio = 1 - train_ratio - test_ratio
    ...
    parser.add_argument('--model', default='WaDIQaM-FR', type=str,
                        help='model name (default: WaDIQaM-FR)')
    ...
    if args.model == 'WaDIQaM-FR' or args.model == 'WaDIQaM-NR':
        args.weighted_average = True
    if args.model == 'DIQaM-FR' or args.model == 'DIQaM-NR':
        args.weighted_average = False

    args.log_dir = args.log_dir + '/EXP{}-{}-{}-lr={}-train'.format(args.exp_id, args.database, args.model, args.lr)
    mkdirs('checkpoints')
    args.trained_model_file = 'checkpoints/{}-{}-EXP{}-lr={}'.format(args.model, args.database, args.exp_id, args.lr)
    mkdirs('results')
    args.save_result_file = 'results/{}-{}-EXP{}-lr={}'.format(args.model, args.database, args.exp_id, args.lr)
